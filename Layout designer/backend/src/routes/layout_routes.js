...
layoutRouter.get("/get-layout-tasks/:id", async (req, res) => {
    try {
        console.log(`[get-layout-tasks] Fetching tasks for layout ID: ${req.params.id}`);
        ...
        if (!layout) {
            console.log(`[get-layout-tasks] Layout not found: ${req.params.id}`);
            return res.status(404).json({ message: "Layout not found" });
        }
        ...
        console.log(`[get-layout-tasks] Found ${waterTasks.length} water tasks and ${fertTasks.length} fert tasks`);
        ...
        console.error(`[get-layout-tasks] Error:`, error);
        res.status(500).json({ message: "Error fetching layout tasks", error: error.message });
    }
});
...
layoutRouter.get("/generate-schedule-tasks/:id", async (req, res) => {
    console.log(`[generate-schedule-tasks] Starting task generation for layout ID: ${req.params.id}`);
    console.log(`[generate-schedule-tasks] OpenAI API Key present: ${process.env.OPENAI_API_KEY ? 'Yes' : 'No'}`);
    
    try {
        // Step 1: Fetch the layout object by its ID
        const layout = await LayoutModel.findById(req.params.id).populate('crop_areas');
        if (!layout) {
            console.log(`[generate-schedule-tasks] Layout not found: ${req.params.id}`);
            return res.status(404).json({ message: "Layout not found" });
        }
        
        console.log(`[generate-schedule-tasks] Layout found with ${layout.crop_areas.length} crop areas`);
        ...
        console.log(`[generate-schedule-tasks] Crop data prepared:`, JSON.stringify(cropData, null, 2));
 
        // Step 3: Initialize OpenAI
        if (!process.env.OPENAI_API_KEY) {
            console.error(`[generate-schedule-tasks] OpenAI API key is missing`);
        ...
        console.log(`[generate-schedule-tasks] Calling OpenAI API...`);
        
        // Step 4: Call OpenAI API with timeout handling
        ...
        console.log(`[generate-schedule-tasks] OpenAI API response received`);
        ...
        try {
            ...
            console.log(`[generate-schedule-tasks] Tasks parsed successfully`);
        } catch (error) {
            console.error(`[generate-schedule-tasks] Failed to parse OpenAI response:`, tasksString);
            console.error(`[generate-schedule-tasks] Parse error:`, error);
        ...
        console.log(`[generate-schedule-tasks] Creating ${parsedTasks.waterTasks.length} water tasks and ${parsedTasks.fertTasks.length} fert tasks in DB`);
        ...
        console.log(`[generate-schedule-tasks] Tasks created in DB. Updating layout...`);
        ...
        console.log(`[generate-schedule-tasks] Success! Layout updated with new tasks`);
        ...
        console.error(`[generate-schedule-tasks] Error occurred:`, error);
        console.error(`[generate-schedule-tasks] Error stack:`, error.stack);
        ...
        if (error.message.includes('timeout')) {
            return res.status(504).json({
                message: "Task generation timed out",
                error: error.message,
