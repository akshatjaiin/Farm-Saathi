...
// Try to load the native Node binding first (faster). If it fails (common on Windows
// without build tools), fall back to the pure-JS `@tensorflow/tfjs` implementation.
let tf;
try {
    tf = await import('@tensorflow/tfjs-node');
    console.log('Loaded @tensorflow/tfjs-node (native backend)');
} catch (err) {
    console.warn('Could not load @tensorflow/tfjs-node, falling back to @tensorflow/tfjs:', err && err.message ? err.message : err);
